...
export const fetchGetData = createAsyncThunk(
    ...
    async (cityName, { rejectWithValue }) => {
        try {
            const geoRes = await fetch(`https://api.openweathermap.org/geo/1.0/direct?q=${cityName}&limit=1&appid=${process.env.REACT_APP_WEATHER_API_TOKEN}`)
            
            if (geoRes.ok) {
                const geoData = await geoRes.json()
                const { lat, lon } = geoData[0]
                try {
                    const res = await fetch(`https://api.openweathermap.org/data/2.5/forecast?lat=${lat}&lon=${lon}&units=metric&appid=${process.env.REACT_APP_WEATHER_API_TOKEN}`)
                    if (res.ok) {
